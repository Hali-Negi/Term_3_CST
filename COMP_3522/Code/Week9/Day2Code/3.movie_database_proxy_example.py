...
class MovieDatabaseProxy:
    ...
    def __init__(self, access_level, name, connect_on_startup=True):
        ...
        if pathlib.Path(name).suffix == ".db" and (
            ...
            self.database = movie_database.MovieDatabase(name, connect_on_startup)
        else:
            raise Exception(
                "Check filename or access level. Guests cannot "
    ...
    def connect(self):
        ...
        if self.access_level == "moderator" or self.access_level == "member":
            ...
            raise Exception("Not authorized. Guests cannot connect to the " "database.")
    ...
    def insert(self, title, director, language, release_year):
        ...
        if self.access_level == "moderator":
            ...
            raise Exception(
                "Not authorized. Guests and members cannot insert "
    ...
    def delete(self, movie_id):
        ...
        if self.access_level == "moderator":
            ...
            raise Exception(
                "Not authorized. Guests and members cannot delete "
                "data, only moderators can."
            )
    ...
    def search(self, title="", director="", language="", release_year=""):
        ...
        """
        if self.access_level == "moderator" or self.access_level == "member":
            return self.database.search(title, director, language, release_year)
        else:
            raise Exception("Not authorized. Guests cannot search the " "database.")
