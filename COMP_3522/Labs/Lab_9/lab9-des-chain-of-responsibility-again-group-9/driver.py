...
from DataInputHandler import DataInputHandler
from DecryptionHandler import DecryptionHandler
...
class Crypto:
    ...
    def __init__(self):
        ...
        self.encryption_start_handler = (
            KeyHandler(InputFileHandler(DataInputHandler(EncryptionHandler(OutputHandler()))))
        )

        self.decryption_start_handler = (
            KeyHandler(InputFileHandler(DataInputHandler(DecryptionHandler(OutputHandler()))))
        )

    def execute_request(self, request: Request):
        if request.encryption_state == CryptoMode.EN:
            request.state = self.encryption_start_handler
        elif request.encryption_state == CryptoMode.DE:
            request.state = self.decryption_start_handler
        else:
            raise Exception("Unknown encryption mode")

        while request.state is not None:
            request.switch()
