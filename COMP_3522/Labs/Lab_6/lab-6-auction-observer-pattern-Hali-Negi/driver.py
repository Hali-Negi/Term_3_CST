
# Hali Imsnpsnsh
# A01424306

"""
Implements the observer pattern and simulates a simple auction.
"""

import random


class Auctioneer:
    """
    The auctioneer acts as the "core". This class is responsible for
    tracking the highest bid and notifying the bidders if it changes.
    """

    def __init__(self):
        self.bidders = []
        self._highest_bid = 0
        self._highest_bidder = None

    def register_bidder(self, bidder):
        """
        Adds a bidder to the list of tracked bidders.
        :param bidder: object with __call__(auctioneer) interface.
        """
        self.bidders.append(bidder)

    def reset_auctioneer(self):
        """
        Resets the auctioneer. Removes all the bidders and resets the
        highest bid to 0.
        """
        self.bidders = []
        self._highest_bid = 0
        self._highest_bidder = None

    def _notify_bidders(self):
        """
        Executes all the bidder callbacks. Should only be called if the
        highest bid has changed.
        """
        for bidder in self.bidders:
            if bidder != self._highest_bidder:
                bidder(self)  # calls bidder.__call__(self)

    def accept_bid(self, bid, bidder):
        """
        Accepts a new bid and updates the highest bid. This notifies all
        the bidders via their callbacks.
        :param bid: a float.
        :precondition bid: should be higher than the existing bid.
        :param bidder: The object with __call__(auctioneer) that placed
        the bid.
        """
        if bid > self._highest_bid:
            previous_bid = self._highest_bid
            previous_bidder = self._highest_bidder
            self._highest_bid = bid
            self._highest_bidder = bidder
            print(f"{bidder.name} bidded {bid} in response to "
                  f"{previous_bidder.name}'s bid of {previous_bid}!")
            self._notify_bidders()



class Bidder:

    def __init__(self, name, budget=100, bid_probability=0.35, bid_increase_perc=1.1):
        self.name = name
        self.bid_probability = bid_probability
        self.budget = budget
        self.bid_increase_perc = bid_increase_perc
        self.highest_bid = 0

    def __call__(self, auctioneer):
        """
        Callable object method. Decides whether this bidder will place
        a new bid based on:
        1. It's not their own bid they're responding to
        2. Their potential new bid won't exceed their budget
        3. They pass the random probability check
        """

        if auctioneer._highest_bidder == self:
            return

        # Calculate what their new bid would be
        potential_bid = auctioneer._highest_bid * self.bid_increase_perc

        # Only bid if within budget AND passes probability check
        if potential_bid <= self.budget and random.random() <= self.bid_probability:
            self.highest_bid = potential_bid
            auctioneer.accept_bid(potential_bid, self)

    def __str__(self):
        return self.name


class Auction:
    """
    Simulates an auction. Is responsible for driving the auctioneer and
    the bidders.
    """

    def __init__(self, bidders):
        """
        Initialize an auction. Requires a list of bidders that are
        attending the auction and can bid.
        :param bidders: sequence type of objects of type Bidder
        """
        self.bidders = bidders
        self.auctioneer = Auctioneer()

        # Register all bidders to the auctioneer (Observer Pattern)
        for bidder in self.bidders:
            self.auctioneer.register_bidder(bidder)


    def simulate_auction(self, item, start_price):
        """
        Starts the auction for the given item at the given starting
        price. Drives the auction till completion and prints the results.
        :param item: string, name of item.
        :param start_price: float
        """
        starting_bidder = Bidder("Starting Bid", budget=float('inf'))
        self.auctioneer._highest_bid = start_price
        self.auctioneer._highest_bidder = starting_bidder

        # Notify all bidders to kick off the chain
        self.auctioneer._notify_bidders()

        # Print results
        winner = self.auctioneer._highest_bidder
        winning_bid = self.auctioneer._highest_bid
        print(f"\nThe winner of the auction is: {winner.name} at ${winning_bid}")

        # Dictionary comprehension: {bidder: highest_bid}
        highest_bids = {bidder: bidder.highest_bid for bidder in self.bidders}

        print("\nHighest Bids Per Bidder")
        for bidder, bid in highest_bids.items():
            print(f"Bidder: {bidder.name}\tHighest Bid: {bid}")


def main():
    bidders = []

    # Hardcoded test bidders
    bidders.append(Bidder("Jojo", 3000, random.random(), 1.2))
    bidders.append(Bidder("Melissa", 7000, random.random(), 1.5))
    bidders.append(Bidder("Priya", 15000, random.random(), 1.1))
    bidders.append(Bidder("Kewei", 800, random.random(), 1.9))
    bidders.append(Bidder("Scott", 4000, random.random(), 2))

    # User input
    item = input("Enter the name of the item being auctioned: ")
    start_price = float(input("Enter the starting price: "))

    add_more = input(
        "\nTest bidders are preloaded (Homa, Charlotte, Pariya, Ashkan, Nabil). Add more? (yes/no): ").strip().lower()

    if add_more == "yes":
        num = int(input("How many additional bidders? "))

        for _ in range(num):
            name = input("  Bidder name: ")
            budget = float(input("  Budget: "))
            prob = float(input("  Bid probability (0-1): "))
            increase = float(input("  Bid increase percentage (e.g. 1.4): "))
            bidders.append(Bidder(name, budget, prob, increase))

        ...

    print("\n\nStarting Auction!!")   # ← must be indented!
    print("*********--------*********")       # ← must be indented!
    my_auction = Auction(bidders)     # ← must be indented!
    my_auction.simulate_auction(item, start_price)  # ← must be indented!

if __name__ == "__main__":
    main()
