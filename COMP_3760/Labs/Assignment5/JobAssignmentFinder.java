...
public class JobAssignmentFinder
{
    ...
    /**
     * Use a greedy algorithm to assign jobs to people.
     * For each person, select the highest-value job that has not already
     * been assigned to someone else.
     * <p>
     * This method does not guarantee the optimal solution, but it is much
     ...
     * This calls getGreedyAssignment() and then sums the values
     * of each person-job pair from the benefit matrix.
     * <p>
     * this recalculates the greedy assignment each time it is called,
     * which is not the most efficient approach.
     *
     * @return
     */
    public int greedyAssignmentTotalValue()
    {
        ArrayList<Integer> assignment = this.getGreedyAssignment();
        int total = 0;

        for (int person = 0; person < this.problemSize; person++)
        {
            total += this.benefitMatrix[person][assignment.get(person)];
        }
        return total;
    }
     ...
     * Problem 1
     * <p>
     * Greedy algorithm that assigns jobs first, selecting the best available person for each job.
     * Iterates through each job and chooses the unused person who provides the highest benefit.
     * <p>
     * Returns an ArrayList where index = person and value = assigned job.
     */
    public ArrayList<Integer> getGreedyAssignmentByJob()
    {
        ArrayList<Integer> assignment = new ArrayList<>();
        ...
        boolean[] usedPersons = new boolean[this.problemSize];
        ...
        for (int job = 0; job < this.problemSize; job++)
        {
            ...
            for (int person = 0; person < this.problemSize; person++)
            {
                if (!usedPersons[person] && this.benefitMatrix[person][job] > bestValue)
                {
                    bestPerson = person;
                    bestValue  = this.benefitMatrix[person][job];
                }
            ...
            assignment.set(bestPerson, job);
            usedPersons[bestPerson] = true;
        }
    ...
    /**
     * Computes the total benefit value of the assignment produced by getGreedyAssignmentByJob().
     * Iterates through each person and sums the benefitMatrix values based on assigned jobs.
     * <p>
     * Returns the total benefit of the assignment.
     */
    public int greedyAssignmentByJobTotalValue()
    {
        ArrayList<Integer> assignment = this.getGreedyAssignmentByJob();
        int total = 0;
     ...
     * Problem 2
     * <p>
     * Greedy algorithm that selects the globally highest-valued (person, job) pair
     * at each step. At every iteration, it scans the entire benefit matrix and chooses
     * the best available assignment, ensuring that each person and each job is used only once.
     *
     * @return an ArrayList where the index represents the person & the value at that index
     * represents the assigned job
     */

    public ArrayList<Integer> getGreedyAssignmentGlobal()
    {
        ArrayList<Integer> assignment = new ArrayList<>();
        for (int i = 0; i < this.problemSize; i++)
        {
        ...
        for (int count = 0; count < this.problemSize; count++)
        {
            int bestPerson = -1;
            int bestJob = -1;
            int bestValue = -1;
            ...
            for (int person = 0; person < this.problemSize; person++)
            {
                for (int job = 0; job < this.problemSize; job++)
                {
                    if (!usedPersons[person] && !usedJobs[job] && this.benefitMatrix[person][job] > bestValue)
                    {
                        bestPerson = person;
                        bestJob    = job;
                        bestValue  = this.benefitMatrix[person][job];
            ...
            assignment.set(bestPerson, bestJob);
            usedPersons[bestPerson] = true;
            usedJobs[bestJob]       = true;
        }
        return assignment;
    ...
    /**
     * Computes the total value of the assignment produced by the global greedy strategy.
     * It sums the benefit values for each (person, job) pair in the assignment.
     *
     * @return the total benefit value of the global greedy assignment
     */

    public int greedyAssignmentGlobalTotalValue()
    {
        ArrayList<Integer> assignment = this.getGreedyAssignmentGlobal();
        int total = 0;
     ...
     * @return
     */
    private int checkValueOfAssignment(ArrayList<Integer> jobAssignment,
                                       int[][] benefitMatrix)
    {
        int total = 0;
